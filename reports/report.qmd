...
run_sql_and_return_html(cnx,"""
SELECT `Short Name`, `Long Name` FROM wdi_country WHERE Region IS NOT NULL AND `Income Group` IS NULL;
""")
...
run_sql_and_return_html(cnx,"""
SELECT Region, COUNT(CASE WHEN `Income Group` = 'High income' THEN 1 END) AS "High income", COUNT(CASE WHEN `Income Group` = 'Upper middle income' THEN 1 END) AS "Upper middle income", COUNT(CASE WHEN `Income Group` = 'Lower middle income' THEN 1 END) AS "Lower middle income", COUNT(CASE WHEN `Income Group` = 'Low income' THEN 1 END) AS "Low income", COUNT(*) AS "Total Countries" FROM wdi_country WHERE Region IS NOT NULL GROUP BY Region ORDER BY Region;
""")
...
SELECT 
  ric.Region,
  ROUND(SUM(CASE WHEN ric.`Income Group` = 'High income' THEN country_count ELSE 0 END) * 100.0 / t.total, 2) AS High_income,
  ROUND(SUM(CASE WHEN ric.`Income Group` = 'Upper middle income' THEN country_count ELSE 0 END) * 100.0 / t.total, 2) AS Upper_middle_income,
  ROUND(SUM(CASE WHEN ric.`Income Group` = 'Lower middle income' THEN country_count ELSE 0 END) * 100.0 / t.total, 2) AS Lower_middle_income,
  ROUND(SUM(CASE WHEN ric.`Income Group` = 'Low income' THEN country_count ELSE 0 END) * 100.0 / t.total, 2) AS Low_income,
  ROUND(SUM(CASE WHEN ric.`Income Group` NOT IN ('High income', 'Upper middle income', 'Lower middle income', 'Low income') THEN country_count ELSE 0 END) * 100.0 / t.total, 2) AS Other,
  ROUND(SUM(country_count) * 100.0 / t.total, 2) AS Row_Total_Percent
FROM region_income_counts ric, total_country_count t
GROUP BY ric.Region
ORDER BY ric.Region;
""")
...
SELECT 
  ...
  ic.country_count,
  t.total,
  ROUND(ic.country_count * 100.0 / t.total, 2) AS percent_of_total
